...
module.exports.handleReply = async function({ event, api, handleReply }) {
  const picture = (await axios.get(`https://i.imgur.com/rik23kQ.jpg`, { responseType: "stream"})).data
...
module.exports.run = async function ({ event, api, args }) {
  ...
  if (!data.gameOn) {
    ...
    if (abc !== "x" && abc !== "o") return api.sendMessage({body: "𝘁𝘁𝘁 + 𝗫 𝗼𝗿 𝗢 𝘁𝘂̀𝘆 𝗯𝗮̣𝗻 𝘀𝗮𝘂 đ𝗼́ 𝗯𝗼𝘁 𝗴𝘂̛̉𝗶 𝗧𝗡 𝘁𝗵𝗶̀ 𝗯𝗮̣𝗻 𝗿𝗲𝗽𝗹𝘆 𝗧𝗡 đ𝗲̂̉ 𝗰𝗵𝗼̣𝗻 𝗼̂ 𝗯𝗮̣𝗻 𝗺𝘂𝗼̂́𝗻 đ𝗶", attachment: (picture)}, threadID, messageID);
    if (abc == "o") {
