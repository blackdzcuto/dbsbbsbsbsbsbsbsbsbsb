module.exports.config = {
  ...
  version: "1.0.0",
  hasPermssion: 2,
  credits: "Nam", // mod thêm ảnh,gif by Tấn An
  description: "auto rep",
  commandCategory: "Tiện ích",
  usages: "VD: autorep a => b BOT sẽ rep khi người dùng nhắn a\n thêm tệp, VD: filter ảnh + link ảnh + key file",
  cooldowns: 5
};
...
const fs = require("fs-extra");
  ...
  const pathFilter = __dirname + "/noprefix/nam.json";
  ...
  for (let word of dataFilter) {
    ...
    let autorep = word.value[Math.floor(Math.random()*word.value.length)];
       return n({body: autorep,attachment: fs.createReadStream(__dirname + `/noprefix/nam_${word.key}.mp4`)}, t, (error, info) => { 
   if (error) return n({body: autorep,attachment: fs.createReadStream(__dirname + `/noprefix/nam_${word.key}.gif`)}, t, (error, info) => {
     if (error) return n({body: autorep,attachment: fs.createReadStream(__dirname + `/noprefix/nam_${word.key}.png`)}, t, (error, info) => {
     if (error) return n({body: autorep,attachment: fs.createReadStream(__dirname + `/noprefix/nam_${word.key}.mp3`)}, t,  
(error, info) => {
...
module.exports.run = async ({ api: a, event: e, args }) => {
  ...
  const dataFilter = require(pathFilter);
    ...
    n(`Đã xóa từ khóa filter ${wordDelete}`, t, m);
     ...
     if (!fs.existsSync(dirMaterial + `nam_${texxt}.png`)) request(args[1]).pipe(fs.createWriteStream(dirMaterial + `nam_${texxt}.png`));
  return n(`Đã thêm tệp hình ảnh vào từ khóa ${texxt}`,t,m);
       }
    else if (args[0] == "gif") {
      let texxt = args.splice(2).join(" ");
     if (!fs.existsSync(dirMaterial + `nam_${texxt}.gif`)) request(args[1]).pipe(fs.createWriteStream(dirMaterial + `nam_${texxt}.gif`));
  return n(`Đã thêm tệp gif vào từ khóa ${texxt}`,t,m);
       }
      else if (args[0] == "mp4") {
        let texxt = args.splice(2).join(" ");
     if (!fs.existsSync(dirMaterial + `nam_${texxt}.mp4`)) request(args[1]).pipe(fs.createWriteStream(dirMaterial + `nam_${texxt}.mp4`));
  return n(`Đã thêm tệp video vào từ khóa ${texxt}`,t,m);
         }
       else if (args[0] == "mp3") {
         let texxt = args.splice(2).join(" ");
     if (!fs.existsSync(dirMaterial + `nam_${texxt}.mp3`)) request(args[1]).pipe(fs.createWriteStream(dirMaterial + `nam_${texxt}.mp3`));
