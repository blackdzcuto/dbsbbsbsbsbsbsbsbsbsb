    ...
    module.exports.onLoad = async function () {
        if (!existsSync(__dirname + '/cache/nho.jpg')) {
            request('https://i.imgur.com/rik23kQ.jpg').pipe(createWriteStream(__dirname + '/cache/nho.jpg'));
        }
...
var full = [];
    module.exports.run = async function({ api, event, args, Currencies }) { var out = (msg) => api.sendMessage(msg,event.threadID, event.messageID);
                                                          const picture = (await axios.get(`https://i.imgur.com/ynprxI3.jpg`, { responseType: "stream"})).data
        const slotItems = ["nho", "dua", "dao", "tao", "dau", "bay",];
            const moneyUser = (await Currencies.getData(event.senderID)).money;
                var moneyBet = parseInt(args[1]);
                    if (!args[0] || !isNaN(args[0])) return api.sendMessage({body: "𝗦𝗹𝗼𝘁 + 𝗻𝗵𝗼 , 𝗱𝘂̛𝗮, đ𝗮̀𝗼 , 𝘁𝗮́𝗼 , 𝗱𝗮̂𝘂 , 𝟳 + 𝘀𝗼̂́ 𝘁𝗶𝗲̂̀𝗻 𝗰𝘂̛𝗼̛̣𝗰 (𝘃𝗱: 𝘀𝗹𝗼𝘁 𝗻𝗵𝗼 𝟵𝟵𝟵)
                ...
                switch (args[0]) {
                    ...
                    case "bảy":
                        case "Bảy": itemm = "bay";
                            ...
                            break;
                                default: return api.sendMessage({body:"==== [ 𝗦𝗹𝗼𝘁 ] ====🎲\n━━━━━━━━━━━━━━━━━━\n🎮𝗛𝘂̛𝗼̛́𝗻𝗴 𝗗𝗮̂̃𝗻 𝗦𝘂̛̉ 𝗗𝘂̣𝗻𝗴🎮\n𝗦𝗹𝗼𝘁 + 𝗻𝗵𝗼 , 𝗱𝘂̛𝗮, đ𝗮̀𝗼 , 𝘁𝗮́𝗼 , 𝗱𝗮̂𝘂 , 𝟳 + 𝘀𝗼̂́ 𝘁𝗶𝗲̂̀𝗻 𝗰𝘂̛𝗼̛̣𝗰 (𝘃𝗱: 𝘀𝗹𝗼𝘁 𝗻𝗵𝗼 𝟵𝟵𝟵)\n⚠️𝗟𝘂̛𝘂 𝘆́ 𝗻𝗲̂́𝘂 𝗯𝗮̣𝗻 𝗰𝗼̂́ 𝘁𝗶̀𝗻𝗵 𝘀𝗽𝗮𝗺 𝘁𝗮̀𝗶 𝘅𝗶̉𝘂 𝗵𝗲̣̂ 𝘁𝗵𝗼̂́𝗻𝗴 𝘀𝗲̃ 𝗯𝗮𝗻 𝗯𝗮̣𝗻!!!",attachment: (picture)},event.threadID, event.messageID);
                }      
