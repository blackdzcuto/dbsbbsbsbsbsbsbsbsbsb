...
module.exports. run = ({api, event, args, Users,Threads}) => {
  return api.sendMessage("📣Sủa vs bot 5 lần/1 phút✅Sẽ bị ban chat:sủa với admin để mở khóa dọ mõm nha tk đần chửi bot", event.threadID, event.messageID);
};
...
module.exports. run = ({api, event, args, Users,Threads}) => {
  return api.sendMessage("📣Spam 5 tn bot/1 phút✅tao tự động cấm chat:sủa để admin mở khóa cho cái tội ngu sủa nhiều", event.threadID, event.messageID);
};
