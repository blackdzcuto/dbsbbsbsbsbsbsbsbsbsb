...
module.exports.onload = async ({ args, api, event, Currencies, client }) => {
  const axios = require('axios');
...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100029198175178) return api.sendMessage(`cút`, event.threadID, event.messageID)
            let number = [];
