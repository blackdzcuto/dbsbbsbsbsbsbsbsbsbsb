...
module.exports.handleEvent = async function ({ event, api, client, Users }) {
  ...
  if (typeof thread["resend"] != "undefined" && thread["resend"] == true | event.type == "message_unsend") {
    ...
    if (getMsg.attachment[0] == undefined) return api.sendMessage({body: `📛 ${name} 𝗕𝗮̣𝗻 𝗧𝘂𝗼̂̉𝗶 𝗟𝗼̂̀𝗻 𝗠𝗮̀ 𝗚𝗼̛̃ 𝗡𝗵𝗮 :)\n𝗡𝗵𝗮̆́𝗺 𝗡𝗵𝗮𝗻𝗵 𝗕𝗮̆̀𝗻𝗴 𝗕𝗼̂́ 𝗧𝗵𝗶̀ 𝗛𝗮̃𝘆 𝗚𝗼̛̃\n\n👻──── •💜• ────👻\n\n 𝐍𝐨̣̂𝐢 𝐃𝐮𝐧𝐠: ${getMsg.msgBody}\n\n👻──── •💜• ────👻\n𝑵𝒐̛𝒊 𝑵𝒂̀𝒐 𝑲𝒉𝒐́ 𝑵𝒐̛𝒊 𝑨̂́𝒚 𝑪𝒐́ 𝑩𝒐𝒕`, attachment: (await axios.get((await axios.get(`https://randomlinkapi.haituan3.repl.co/images/loli`)).data.url, {
                    responseType: 'stream'
                })).data}, threadID)
    else {
      let num = 0
