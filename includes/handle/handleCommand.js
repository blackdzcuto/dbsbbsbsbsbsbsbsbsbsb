module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function ({ event }) {
    ...
    if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {
      const res = await axios.get(`https://docs-api.blackbotloapi.repl.co/images/phongcanh`);
...
if (noleak == 'Sunday') noleak = '𝐂𝐡𝐮̉ 𝐍𝐡𝐚̣̂𝐭'
  ...
  if (noleak == 'Saturday') noleak = '𝐓𝐡𝐮̛́ 𝐁𝐚̉𝐲'
      const res = await axios.get(`http://botblackcute.ml/api/hearing.php`); 
...
url: (await global.nodemodule["axios"]('https://docs-api.blackbotloapi.repl.co/images/phongcanh')).data.data,
method: "GET",
responseType: "stream"
})).data
...
responseType: "stream"
    ...
    try {
      ...
      if (DeveloperMode == !![])
        logger(global.getText("handleCommand", "executeCommand", time, commandName, senderID, threadID, args.join(" "), (Date.now()) - dateNow), "[ 𝐇𝐚𝐫𝐢𝐧 ]");
      return;
    } catch (e) {
