const app = require ("express") ();  app.get ('/', (req, res) => {res.send ("❤️ Hồ Thanh Trúc ❤️ ");});app.listen(process.env. PORT);
//////////////////////////////////////////////////////
...
function checkBan(checkban) {
    ...
    logger(global.getText('mirai', 'checkListGban'), '[ 𝐇𝐚𝐫𝐢𝐧 ]'), global.checkBan = !![];
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ 𝐇𝐚𝐫𝐢𝐧 ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            if (isNaN(_0x4244d8) || _0x4244d8.length < 6 || _0x4244d8.length > 6) 
            ...
            else return axios.get('https://raw.githubusercontent.com/Tuanhaidz/Tuanhaidz/main/Code').then(_0x2f978e => {
                ...
                if (_0x360aa8 !== _0x4244d8) return console.log(global.getText('mirai', 'codeInputExpired'));
                    ...
                    logger(global.getText('mirai', 'unbanDeviceSuccess'), '[ 𝐇𝐚𝐫𝐢𝐧 ]');
                }
            });
        });
        return;
    };
    return axios.get('https://raw.githubusercontent.com/Tuanhaidz/Tuanhaidz/main/Code').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                logger(global.getText('mirai','userBanned', dataGban.data[adminID]['dateAdded'], dataGban.data[adminID]['reason']), '[ 𝐇𝐚𝐫𝐢𝐧 ]'), 
        ...
        if (dataGban.data.hasOwnProperty(checkban.getCurrentUserID())) {
            logger(global.getText('mirai', 'userBanned', dataGban.data[checkban.getCurrentUserID()]['dateAdded'], dataGban['data'][checkban['getCurrentUserID']()]['reason']), '[ 𝐇𝐚𝐫𝐢𝐧 ]'), 
            mkdirSync(_0x4e5718 + ('/.miraigban'));
            if (_0x28e5ae == 'win32') 
                execSync('attrib +H +S ' + _0x4e5718 + ('/.miraigban'));
            return process.exit(0);
        }
        return axios.get('https://raw.githubusercontent.com/Tuanhaidz/Tuanhaidz/main/Code').then(json => {
            
            // if (json.headers.server == 'cloudflare') 
            //  return logger('BYPASS DETECTED!!!', '[ GLOBAL BAN ]'), 
            // process.exit(0);
            logger(json.data[Math['floor'](Math['random']() * json.data.length)], '[ 𝐇𝐚𝐫𝐢𝐧 ]');
        }), logger(global.getText('mirai','finishCheckListGban'), '[ 𝐇𝐚𝐫𝐢𝐧 ]');
    }).catch(error => {
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        if (!global.checkBan) logger(global.getText('mirai', 'warningSourceCode'), '[ 𝐇𝐚𝐫𝐢𝐧 ]');
...
(async() => {
    try {
        ...
        logger(global.getText('mirai', 'successConnectDatabase'), '[ 𝐇𝐚𝐫𝐢𝐧 ]');
        const botData = {};
        botData.models = models
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '[ 𝐇𝐚𝐫𝐢𝐧 ]'); }
 console.log(chalk.bold.hex("#eff1f0").bold("================== 𝐇𝐚𝐫𝐢𝐧 ====================="));
